  ...
  if (getUserId && getBarangId) {
    try {
      ...
      res.status(201).json({
        ...
        data: {
          id_borrow: result.id_peminjaman,
...
export const pengembalianBarang = async (req, res) => {
  ...
  try {
    // cari data sesuai id_peminjaman
    ...
    if (cekBorrow.status === "dipinjam") {
      ...
      if (!item) {
        return res.status(404).json({
          msg: `Barang dengan id_barang ${cekBorrow.id_barang} tidak ditemukan`,
        });
      }
      ...
      res.status(400).json({ msg: "Barang tidak dalam status 'dipinjam'" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json({
